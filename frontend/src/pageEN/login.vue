				...
				<el-col :span="6" :offset="9"   :xs="6" :md="6" :lg="6" :sm="6">
					<el-card class="box-card" style="margin-top:40px">
						<el-form ref="form" :model="form" action="" role="form" :rules="rules2" class="demo-ruleForm" @keyup.enter.native="dj('form')">
							...
							<el-col :span="24"  :xs="24" :md="24" :lg="24" :sm="24">
								<el-form-item>
									<el-button class="el-col el-col-xs-24 el-col-md-24 el-col-sm-24 el-col-lg-24" type="primary"  @click="dj('form')">Login</el-button>
								</el-form-item>
							</el-col>
						</el-form>
					</el-card>
  	...
  	methods: {
  		dj: function(formname) {
  			...
  			this.$refs[formname].validate((valid) => {
  				if (valid) {
  					this.$store.dispatch('UserLogin_en', this.form)
  					.then(function success(){
			        	notifi('Successfully login', 'Welcome,'+context.$store.state.login.username, 'success', context)
			        	context.$store.dispatch('UpdateUserInfo')
			        	context.$store.dispatch('getNotification')
			        }, function fail(){
			        	notifi('Login failed', 'Wrong username or password', 'error', context)
			        })
  				} else {
  					notifi('Login failed', 'Invalid username or password', 'error', context)
  					return false;
  				}
  			});
  		}
